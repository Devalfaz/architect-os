# The Constitution — Agent Rules C1–C35
...
**C7 — No "while I'm in here."** Do not refactor, optimize, or improve code outside the ticket's scope. File a new ticket.
...
**C15 — Magic numbers must be named constants.** No unexplained literals in logic. `0`, `1`, `-1`, `null`, `""`, `[]` are not magic numbers.
...
**C20 — Flaky tests are broken code.** Fix or quarantine with `test.skip` and a ticket. Never merge a flaky test.
...
3. **CodeRabbit:** configured to flag constitution violations by rule ID.
4. **Human review:** the [10-minute rubric](pr-review-rubric.md) explicitly checks constitution compliance.

## Amending the constitution

Constitution changes are ADRs: `docs/adr/NNNN-constitution-amendment.md` with problem, real example, compliance check, effective date. PR-reviewed. Agent may not amend without human approval.
